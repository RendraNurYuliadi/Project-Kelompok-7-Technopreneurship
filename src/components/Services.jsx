...
import DesignServices from './OurTeam/Service/Design.png'
import WebDevelopment from './OurTeam/Service/WebDev.png'
import DataScraping from './OurTeam/Service/Scrap.png'
        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          {services.map((service,idx)=> (
            <motion.div key={service.title} whileHover={{y:-8}} whileInView={{opacity:1,y:0}} initial={{opacity:0,y:20}} viewport={{once:true}} transition={{duration:0.5,delay:idx*0.1}} data-aos="fade-up" data-aos-delay={idx*100} className="glass p-8 rounded-3xl border border-white/10 shadow-glass relative overflow-hidden">
              ...
              <div className="mb-6 h-48 w-full rounded-2xl overflow-hidden bg-white/5 flex items-center justify-center">
                {service.image ? (
                  <img src={service.image} alt={service.title} className="w-full h-full object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center rounded-2xl bg-slate-900 text-slate-400 text-center px-4">
                    <span className="text-sm">Preview produk akan tersedia segera.</span>
                  </div>
                )}
              </div>
